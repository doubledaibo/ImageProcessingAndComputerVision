...
int main() {
	...
	*/
    ...
    params.term_crit = cvTermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 1000000, 1e-10);

	const float scales[] = {0.25, 0.3, 0.5, 1}; // Multiple scales
	const float threshold[] = {3.5, 4.5, 6.5, 6.5}; // The threshold for each scales
	const bool drawBox = true; // Switch to draw the bounding boxes.
	const float overlap = 0.3; // The overlap ration for NMS
