...
from estimateHSflow import estimateHSflow # include this package if you finish it
from matplotlib import pyplot as plt

if __name__ == '__main__':
    # read 2 input frames
    dataname = "Dimetrodon"
    img1 = cv2.imread('./data/other-data/' + dataname + '/frame10.png').astype('float32')
    img2 = cv2.imread('./data/other-data/' + dataname + '/frame11.png').astype('float32')
    # input ground truth optical flow
    flow_gt = ft.readFlowFile('./data/other-gt-flow/' + dataname + '/flow10.flo')
    flow_gt[flow_gt>1e9] = np.nan

    img1g = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
    img1g = np.round(img1g)
    img2g = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
    img2g = np.round(img2g)
    ...
    ft.writeFlowFile(uv, dataname + ".flo")
    cv2.imwrite(dataname + ".jpg", ft.flowToColor(uv)[:, :, [2, 1, 0]])
